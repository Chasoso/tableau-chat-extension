...
test.skip(process.env.PW_VITE_AUTH_REQUIRED !== "true", "requires PW_VITE_AUTH_REQUIRED=true");

test.describe("auth overlay visual", () => {
  test("shows unauthenticated overlay over the initial screen", async ({ page }) => {
    ...
    await expect(page.locator(".chat-panel")).toHaveScreenshot("chat-panel-unauth-overlay.png", {
      animations: "disabled",
      caret: "hide",
      scale: "css",
