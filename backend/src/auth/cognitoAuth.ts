...
export async function authenticateRequest(headers: Record<string, string | undefined> | undefined): Promise<AuthResult> {
  ...
  if (!config.auth.cognitoUserPoolId || !config.auth.cognitoClientId || !config.auth.cognitoRegion) {
    return { ok: false, statusCode: 403, message: "Authentication is not configured." };
  }
  ...
  try {
    ...
    if (!decodedHeader?.kid) {
      return { ok: false, statusCode: 401, message: "Invalid authentication token." };
    }
    ...
    if (!isExpectedClient(payload, config.auth.cognitoClientId)) {
      return { ok: false, statusCode: 403, message: "Token audience is not allowed." };
    }
...
async function getCognitoKeys(): Promise<Map<string, KeyObject>> {
  ...
  cachedKeys = new Map(
    jwks.keys.map((key) => [key.kid, createPublicKey({ key: key as unknown as NodeJsonWebKey, format: "jwk" })]),
  );
  return cachedKeys;
}
