...
| `VITE_API_BASE_URL` | API base URL embedded into the frontend build. It is public at runtime but should not appear in Actions logs. |
...
1. Preferred: create stable custom domains first and store those domains in `VITE_API_BASE_URL`, `EXTENSION_SOURCE_URL`, and `CORS_ALLOWED_ORIGIN`.
2. Bootstrap: deploy once with temporary values, retrieve the generated endpoints manually in AWS Console or CLI outside Actions logs, then update GitHub Secrets and rerun.

The workflow intentionally does not print generated endpoints.
...
| `VITE_API_BASE_URL` | フロントエンドビルドへ埋め込まれる API base URL。実行時には公開情報ですが Actions ログには出さない方針です。 |
...
1. 推奨: 先に安定した custom domain を作成し、その domain を `VITE_API_BASE_URL`、`EXTENSION_SOURCE_URL`、`CORS_ALLOWED_ORIGIN` に保存する。
2. Bootstrap: 一時値で一度デプロイし、生成された endpoint を AWS Console または GitHub Actions ログ外の CLI で手動取得し、GitHub Secrets を更新して再実行する。

ワークフローは生成された endpoint を意図的に出力しません。
