...
| `TABLEAU_CONTEXT_PROVIDER` | `mock` | `mock`, `direct-api`, or `mcp`. Use non-mock providers only after auth is verified. |
| `AUTH_REQUIRED` | `false` | Enables backend Cognito JWT validation for chat API. |
| `VITE_AUTH_REQUIRED` | `AUTH_REQUIRED` | Enables frontend Cognito Hosted UI login gate. |
| `COGNITO_REGION` | - | Cognito User Pool region. |
| `TABLEAU_MCP_TRANSPORT` | `http` | MCP transport mode. Only HTTP stub is implemented in this PoC. |
| `TABLEAU_MCP_AUTH_MODE` | `none` | MCP authentication mode placeholder. |
| `TABLEAU_MCP_TIMEOUT_MS` | `5000` | MCP request timeout in milliseconds. |
...
| `TABLEAU_CONTEXT_PROVIDER` | `mock` | `mock`、`direct-api`、`mcp`。非 mock provider は認証確認後に使います。 |
| `AUTH_REQUIRED` | `false` | Chat API の Cognito JWT 検証を有効化します。 |
| `VITE_AUTH_REQUIRED` | `AUTH_REQUIRED` | フロントエンドの Cognito Hosted UI login gate を有効化します。 |
| `COGNITO_REGION` | - | Cognito User Pool region。 |
| `TABLEAU_MCP_TRANSPORT` | `http` | MCP transport mode。この PoC では HTTP stub のみ実装しています。 |
| `TABLEAU_MCP_AUTH_MODE` | `none` | MCP authentication mode の placeholder。 |
| `TABLEAU_MCP_TIMEOUT_MS` | `5000` | MCP request timeout milliseconds。 |
